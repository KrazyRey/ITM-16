...
// =============================================
const questions = [
  { type: 'mc', text: 'What is data presentation?', options: ['Collecting raw data', 'Organizing and summarizing information to make it meaningful', 'Destroying unnecessary data', 'Memorizing information'], letters: 1 },
  { type: 'mc', text: 'Which method uses sentences and paragraphs to explain information?', options: ['Graphical', 'Tabular', 'Textual', 'Statistical'], letters: 2 },
  { type: 'mc', text: 'Textual presentation is most suitable for:', options: ['Large datasets', 'Complex numerical comparisons', 'Small amounts of data', 'Continuous data only'], letters: 2 },
  { type: 'mc', text: 'Tabular presentation organizes data into:', options: ['Pictures and symbols', 'Rows and columns', 'Paragraphs and essays', 'Bars and slices'], letters: 1 },
  { type: 'mc', text: 'Which graph is best used to show trends over time?', options: ['Pie Chart', 'Bar Graph', 'Line Graph', 'Pictograph'], letters: 2 },
  { type: 'mc', text: 'A graph that compares categories using rectangular bars is called a:', options: ['Histogram', 'Pie Chart', 'Line Graph', 'Bar Graph'], letters: 3 },
  { type: 'mc', text: 'Which graphical tool displays parts of a whole using slices?', options: ['Pie Chart', 'Histogram', 'Flowchart', 'Control Chart'], letters: 0 },
  { type: 'mc', text: 'A Pareto Chart is best described as:', options: ['A circular graph', 'A bar graph in ascending order', 'A bar graph in descending order with a cumulative line', 'A chart showing process steps'], letters: 2 },
  { type: 'mc', text: 'Which tool is also known as a Fishbone Diagram?', options: ['Control Chart', 'Flowchart', 'Run Chart', 'Cause-and-Effect Diagram'], letters: 3 },
  { type: 'mc', text: 'Which graphical tool monitors a process over time to determine stability?', options: ['Run Chart', 'Control Chart', 'Pie Chart', 'Pictograph'], letters: 1 },
  { type: 'tf', text: 'Graphical presentation makes complex data easier to understand.', letters: true },
  { type: 'tf', text: 'Tables are not useful for numerical data.', letters: false },
  { type: 'tf', text: 'A Run Chart helps track changes and trends over time.', letters: true },
  { type: 'tf', text: 'Flowcharts use symbols and arrows to represent steps in a process.', letters: true },
  { type: 'tf', text: 'Quality improvement focuses on reducing errors and increasing efficiency.', letters: true },
];
...
function selectAnswer(qIdx, value) {
  ...
  answered[qIdx] = true;
  const isCorrect = value === q.letters;
  ...
  if (q.type === 'mc') {
    for (let j = 0; j < 4; j++) {
      ...
      if (!btn) continue;
      btn.disabled = true;
      if (j === q.letters) btn.classList.add('qi-opt-correct');
    ...
    ['true', 'false'].forEach(v => {
      ...
      if (boolVal === q.letters) btn.classList.add('qi-opt-correct');
...
function finishQuiz() {
  quizFinished = true;
  const score = userAnswers.reduce((acc, ans, i) => ans === questions[i].letters ? acc + 1 : acc, 0);
  ...
  const summary = questions.map((q, i) => {
    const correct = userAnswers[i] === q.letters;
    let answerText = '';
    if (q.type === 'mc') {
    ...
    let correctText = '';
    if (q.type === 'mc') correctText = q.options[q.letters];
    else correctText = q.letters ? 'TRUE' : 'FALSE';
    return `
      <div class="qi-summary-item ${correct ? 'qi-s-correct' : 'qi-s-wrong'}">
        ...
        <div class="qi-s-body">
          ...
          ${!correct ? `<div class="qi-s-a">Your letters: <strong>${answerText}</strong> · Correct: <strong>${correctText}</strong></div>` : `<div class="qi-s-a">Your letters: <strong>${answerText}</strong></div>`}
        </div>
      </div>`;
  }).join('');
...
function updateDots() {
  questions.forEach((_, i) => {
    ...
    if (answered[i]) {
      d.classList.add(userAnswers[i] === questions[i].letters ? 'qi-dot-correct' : 'qi-dot-wrong');
    }
  });
}
